...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL)
{
    ...
    $product_count = $t->db->select('count(DISTINCT(p.id)) as total , GROUP_CONCAT(pa.attribute_value_ids) as attr_value_ids')->join(" categories c", "p.category_id=c.id ", 'LEFT')->join('`product_variants` pv', 'p.id = pv.product_id', 'LEFT')->join('`product_attributes` pa', ' pa.product_id = p.id ', 'LEFT');
...
function validate_stock($product_variant_ids, $qtns)
{
    ...
    for ($i = 0; $i < count($product_variant_ids); $i++) {
        $res = $t->db->select('p.*,pv.*,pv.id as pv_id,p.stock as p_stock,pv.stock as pv_stock')->where('pv.id = ', $product_variant_ids[$i])->join('products p', 'pv.product_id = p.id')->get('product_variants pv')->result_array();
        ...
        if (($res[0]['stock_type'] != null && $res[0]['stock_type'] != '')) {
            ...
            if ($res[0]['stock_type'] == 0) {
                if ($res[0]['p_stock'] != null && $res[0]['p_stock'] != '') {
                    ...
                    if ($stock < 0 || $res[0]['availability'] == 0) {
                        $error = true;
                        break;
                    }
                }
...
{
    $t = &get_instance();
    $t->db->select('(select sum(qty)  from cart where user_id="' . $user_id . '" and qty!=0  and  is_saved_for_later = "' . $is_saved_for_later . '" ) as total_items,(select count(c.id) from cart c join product_variants pv on c.product_variant_id=pv.id where user_id="' . $user_id . '" and qty!=0 and  is_saved_for_later = "' . $is_saved_for_later . '" ) as cart_count,`c`.qty,p.is_prices_inclusive_tax,p.cod_allowed,p.minimum_order_quantity,p.slug,p.quantity_step_size,p.total_allowed_quantity, p.name, p.image,p.short_description,`c`.user_id,pv.*,tax.percentage as tax_percentage,tax.title as tax_title');
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null)
{
    ...
    $count_res = $t->db->select(' COUNT(distinct o.id) as `total`')->join(' `users` u', 'u.id= o.user_id', 'left')->join(' `order_items` oi', 'o.id= oi.order_id', 'left')->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
        ->join('products p', 'pv.product_id=p.id', 'left')->join('addresses a', 'a.id=o.address_id', 'left');
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        $final_total = $order_details[$i]['final_total'] - $order_details[$i]['wallet_balance'] - $order_details[$i]['promo_discount'] - $order_details[$i]['discount'];
